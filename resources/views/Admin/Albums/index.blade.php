        ...
        <div class="col-xxl-12">
            <div class="card stretch stretch-full">
                <div class="card-header d-flex flex-wrap justify-content-between align-items-center gap-3">
                    ...
                    <a href="{{ route('admin.gallery.create') }}" class="btn btn-primary ms-auto">Add New Gallery</a>
                </div>
                            ...
                            <tbody>
                                @foreach ($albums as $album)
                                    <tr>
                                        ...
                                        <td>
                                            <div class="d-flex align-items-center gap-3">
                                                ...
                                                <a href="javascript:void(0);">
                                                    <span class="d-block">{{ $album->title }}</span>
                                                </a>
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <span class="badge {{ $album->status ? 'bg-soft-success text-success' : 'bg-soft-danger text-danger' }}">
                                                {{ $album->status ? 'Active' : 'Inactive' }}
                                            </span>
                                        </td>
                                        <td class="text-end">
                                            <div class="hstack gap-2 justify-content-end">
                                                {{-- <a href="{{ route('admin.album.show', $album->id) }}" class="avatar-text avatar-md"> --}}
                                                <a href="#" class="avatar-text avatar-md">
                                                    <i class="feather-eye"></i>
                                                </a>
                                                <a href="{{ route('admin.gallery.edit',$album->id) }}" class="avatar-text avatar-md">
                                                    <i class="feather-edit"></i>
                                                </a>

                                                <!-- Delete Form -->
                                                <form action="{{ route('admin.gallery.destroy', $album->id) }}" method="POST" style="display:inline;">
                                                    @csrf
                                                    @method('DELETE')
                ...
                <div class="card-footer">
                    ...
                    @if ($albums->hasPages())
                        <ul class="list-unstyled d-flex align-items-center gap-2 mb-0 pagination-common-style">
                            ...
                            @else
                                <li>
                                    <a href="{{ $albums->previousPageUrl() }}"><i class="bi bi-arrow-left"></i></a>
                                </li>
                            @endif

                            <!-- Page Numbers -->
                            @foreach ($albums->links()->elements as $element)
                                ...
                                @if (is_array($element))
                                    @foreach ($element as $page => $url)
                                        <li>
                                            <a href="{{ $url }}" class="{{ ($page == $albums->currentPage()) ? 'active' : '' }}">{{ $page }}</a>
                                        </li>
                                    @endforeach
                                @endif
                            @endforeach
                            ...
                            @if ($albums->hasMorePages())
                                <li>
                                    <a href="{{ $albums->nextPageUrl() }}"><i class="bi bi-arrow-right"></i></a>
                                </li>
                            @else
                                <li class="disabled">
